...
struct Localizer {
    ...
    private static let table: [String: [AppLanguage: String]] = [
        ...
        "safetyNotice": [.simplifiedChinese: "启用后，FanPilot 会安装一个本地授权 helper，并通过它访问 AppleSMC。FanPilot 只会提高最低风扇转速，不会低于 Apple 默认最低值；任何读取或写入失败都会自动回到监控模式。", .traditionalChinese: "啟用後，FanPilot 會安裝一個本機授權 helper，並透過它存取 AppleSMC。FanPilot 只會提高最低風扇轉速，不會低於 Apple 預設最低值；任何讀取或寫入失敗都會自動回到監控模式。", .english: "After enabling, FanPilot installs a local authorized helper and uses it to access AppleSMC. FanPilot only raises fan speed targets and never goes below Apple defaults. Any read or write failure returns the app to monitoring mode."],
